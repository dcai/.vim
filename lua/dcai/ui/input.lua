--- copied from https://github.com/liangxianzhe/floating-input.nvim/tree/main
local M = {}
...
function M.input(opts, on_confirm, win_config)
  ...
  if prompt == 'New Name: ' then
    win_config =
      vim.tbl_deep_extend('force', win_config, M.under_cursor(win_config.width))
  else
    win_config = vim.tbl_deep_extend(
      'force',
      win_config,
      M.window_center(win_config.width)
    )
  end
